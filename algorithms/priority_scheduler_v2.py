...
from .step_planners.dkk_atp_planner_v2 import DkkAtpPlannerV2 as DkkAtpPlanner

class PriorityBasedSchedulerV2(PlanningAlgorithm):
    def solve(
        self,
        ...
        remaining_counts: Dict[str, int] = {}  # product_type -> count
        ...
        def _build_normal_jobs(product, all_jobs, product_targets, remaining_counts):
            ...
            # remaining_counts:
            # - Aylık modda: aylık hedefin tamamı (hafta no sabit 0; doğrudan aylık üzerinden priority)
            # - Haftalık/custom modda: bu haftanın hedefi
            ...
            if period == "monthly":
                ...
                # Gelecek hafta(lar): kalan üretimi week_number+1..3 arası eşit dağıt
                # (ay sonunda durur, week 4+ üretilmez)
                rem = full_remaining - target
                pull_week = week_number + 1
                last_week = 3  # ay 0..3 arası
        ...
        if excel_products:
            ...
            # excel_seq: aynı tip/öncelik/ready_time'lı jobların FIFO/sort
            # tie-break'i için Excel satır sırası — atlamasız 1, 2, 3, ...
            excel_seq = 0
        ...
        # Priority snapshot mekanizması: simülasyon sayfası slider zamanına göre lookup yapacak
        ...
        # Gelecek hafta işlerini assembly kuyruğundan ayır ve target_week'e göre grupla:
        # Bu haftanın işleri bitmeden hafta 2 release olmaz; hafta 2 bitmeden hafta 3 release olmaz...
        ...
        # Hafta sırasına göre sıralı buffer: [(week, jobs), ...]
        pending_future_buckets: List[Tuple[int, List[Job]]] = sorted(
            _future_by_week.items(), key=lambda kv: kv[0]
        )

        # Kullanıcı tanımlı ek üretim listesi varsa joblarını üretip kuyruğa ekle.
        # Sıra koruması: her item'a azalan priority verilir; assembly_planner sıraya göre çeker.
        # Auto ek üretim mantığı bu durumda DEVRE DIŞI kalır (aşağıdaki main loop'ta gate var).
        ...
        if _user_extra_list:
            ...
            for _idx, _item in enumerate(_user_extra_list):
                ...
                if not _pname_raw or _qty <= 0:
                    continue
                # Mevcut ürün listesinde eşleşme varsa o ürünün type/name'ini kullan;
                # yoksa kullanıcının manuel girdiği değerleri kullan ("Diğer" akışı).
                ...
                _prio = -1.0 - _idx * 0.001  # azalan priority — sıra korunur
                for _ in range(_qty):
            ...
            # Ek üretim joblarını ayrı bir bekletme listesine al — assembly kuyruğuna
            # SADECE bu hafta + gelecek hafta (tüm aylık hedef) işleri bittiğinde release edilirler.
        ...
        while not all(job.is_completed for job in all_jobs) and event_queue.has_events():
            ...
            # --- DİNAMİK HAFTALIK ÖNCELİK YENİDEN HESAPLAMASI ---
            # Simülasyon içinde her 7 gün geçtiğinde hafta artar ve yeni öncelik durumu doğar
            # Aylık modda hafta sabit kalır (kullanıcı isteği) — bu blok atlanır
            if period != "monthly" and (current_time - last_calc_date).days >= 7:
                ...
                # full_remaining_now = bu tip için kalan tamamlanmamış iş sayısı
                # weeks_left = 4 - current_sim_week (en az 1)
                # new_weekly_target = ceil(full_remaining_now / weeks_left)
            ...
            # Gelecek hafta işlerini hafta hafta serbest bırak:
            # Mevcut kuyruk boşaldığında bir SONRAKİ haftanın bucket'ı release edilir.
            # Sıra: bu hafta bitti → hafta 2 release → hafta 2 bitti → hafta 3 release → ...
            ...
            # Ek üretim release: TÜM aylık hedef bittiğinde (bu hafta + tüm gelecek hafta bucket'ları boş)
            ...
            if e:
                ...
                # Assembly bitti → remaining azaldı → öncelikleri hemen yenile
                new_priorities, _ = PriorityCalculator.calculate_all(
                    all_jobs, project_data.production_time_data, start_date, end_date,
                    remaining_counts, priority_overrides, week_number=current_sim_week
                )
                for job in all_jobs:
                    if not job.is_completed:
                        job.priority_score = new_priorities.get(job.job_id, job.priority_score)
                # Snapshot: assembly batch'inin bittiği zamanda priority durumunu kaydet
            ...
            if _dkk_non_work:
                ...
                # v2 DkkAtpPlanner farklı arayüz, product_index_map sadece v1 group_builder yolunda gerekli
        ...
        for rt in rem_targets:
            ...
            summary_data_to_log["products"].append({
                ...
                "excel_count": rt.scheduled_count if is_excel else 0,
        ...
        return PlanningResult(
            ...
            algorithm_used="PriorityBasedSchedulerV2 (Excel)" if excel_products else "PriorityBasedSchedulerV2",
    ...
    def _get_shift_boundaries(self, current_time: datetime, project_data: AppState, step_name: str="Assembly") -> Tuple[Optional[datetime], Optional[datetime]]:
        ...
        for idx, s in enumerate(parsed_shifts):
            ...
            if not is_cross_midnight:
                ...
                # V3 gibi gece yarısını geçen: 23:00 → 07:00
                if current_t >= start_t and idx in today_allowed:
                    # Başlangıç-gün dilimi (örn. Cmt 23:30 — Cmt'nin V3'ü)
                    in_shift = True
                    end_date_val = current_date + timedelta(days=1)
                elif current_t < end_t and idx in yesterday_allowed:
                    # Bitiş-gün dilimi (örn. Pzr 02:00 — Cmt'nin V3'ü hala sürüyor)
                    in_shift = True
                    # end_date_val = current_date (vardiya bugün biter)
    ...
    def _compute_pause_aware_segments(
        ...
        """Batch'i vardiya kesintilerine göre segmentlere böl.

        Assembly insan işi: vardiya bitince batch durur, sonraki vardıyada devam.
        Toplam `duration_hours` saatlik işi vardıya kesintilerine bölerek
        [(seg_start, seg_end), ...] listesi döndürür. Her segment kesintisiz
        bir vardiya dilimi içindedir; aralarda pause olabilir.
        """
        from datetime import timedelta
    ...
    def _compute_pause_aware_end_time(
        self, start: datetime, duration_hours: float,
        project_data: AppState, step_name: str = "Assembly"
    ) -> datetime:
        """Batch'in pause-aware bitiş zamanını döndürür (segmentlerin son end'i)."""
        segs = self._compute_pause_aware_segments(start, duration_hours, project_data, step_name)
        if not segs:
            from datetime import timedelta
